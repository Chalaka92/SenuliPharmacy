    ...
    public class Delete
    {
        ...
        public class Handler : IRequestHandler<Command>
        {
            ...
            public async Task<Unit> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                itemBatch.ItemCount += orderItemBatch.ItemCount;
                itemBatch.ReorderCount -= orderItemBatch.ItemCount;

                _context.Remove(orderItemBatch);

                //Update Order
                var order = await _context.Orders.FindAsync(orderItemBatch.OrderId);
                order.TotalAmount -= (orderItemBatch.ItemPrice * orderItemBatch.ItemCount);
